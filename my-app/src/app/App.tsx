        ...
        <div className="flex gap-4 flex-col p-10">
            ...
            <div className="w-full"> 
              ...
              <select value={selectedCategory} name="category"
                      onChange={(e)=>changeCategory(e.target.value)}
                       className="flex-1 text-gray-800 outline-none bg-transparent cursor-pointer">
          ...
          {/*-------- SUB CATEGORIES --------*/}
            <div className="w-full"> 
              ...
              <select value={selectedSubCategory} name="subcategory" disabled={!selectedCategory}
                      onChange={(e)=>changeSubCategory(e.target.value)}
                       className={`flex-1 outline-none bg-transparent ${selectedCategory ? 'text-gray-800 cursor-pointer' : 'text-gray-400'}`}>
          ...
          {/*-------- BRANDS --------*/}
            <div className="w-full"> 
              ...
              <select value={selectedBrand} name="brand" disabled={!selectedSubCategory}
                      onChange={(e)=>changeBrand(e.target.value)}
                       className={`flex-1 outline-none bg-transparent ${selectedSubCategory ? 'text-gray-800 cursor-pointer' : 'text-gray-400'}`}>
          ...
          {/*-------- PRODUCT --------*/}
            <div className="w-full"> 
              ...
              <select value={selectedProduct} name="products" disabled={!selectedBrand}
                      onChange={(e)=>changeProduct(e.target.value)}
                       className={`flex-1 outline-none bg-transparent ${selectedBrand ? 'text-gray-800 cursor-pointer' : 'text-gray-400'}`}>
            ...
            <button onClick={()=>reset()} className="mt-5 cursor-pointer text-gray-800 bg-blue-300 font-bold tracking-wider py-3.5 rounded-xl border border-blue-500">RESET</button>
          </form>
        </div>
    </section>
